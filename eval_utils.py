    ...
    if task == 'aste':
        ...
        for s in sents:
            ...
            try:
                ...
                if sp != sp2:
                    print(f'Sentiment polarity of AC({sp}) and AT({sp2}) is inconsistent!')
...
def compute_f1_scores(pred_pt, gold_pt):
    ...
    scores = {'precision': precision, 'recall': recall, 'f1': f1,'n_gold': n_gold,
        'n_pred': n_pred,
        'n_tp': n_tp}
...
def compute_scores(pred_seqs, gold_seqs, sents):
    ...
    result_str = f"number of gold spans: {scores['n_gold']}, predicted spans: {scores['n_pred']}, hit: {scores['n_tp']}\n"
    result_str += f"precision: {scores['precision']:.4f}, recall: {scores['recall']:.4f}, F1 = {scores['f1']:.4f}"
    
    print(result_str)
    scores['result_str'] = result_str  
